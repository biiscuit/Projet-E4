...
<?php
	if(count($lstProdByFourn) == 0){
		echo '<td width:100% class="text-center"> Aucun Résultat </td>';
	}
	else{
		echo '<tr>';
		foreach ($lstProdByFourn as $value) {
			$compteur++;
?>
	<td width=33%> 
...
?>
			...
			?>
					<a class="btn btn-primary" href="index.php?section=2&action=2&id_fourn=<?= $value->getIdFourn() ?>&page=<?= $num_page ?>"> <?= $num_page ?></a>
			<?php
				}
			?>
